/*
 * To change this license header, choose License Headers in Project Properties.
 * To change this template file, choose Tools | Templates
 * and open the template in the editor.
 */
package com.shebadigital.cms.common;

import java.text.DecimalFormat;

/**
 *
 * @author riad
 */
public class NumberConverter {
    
    
    public static String numberToWord(double amount) {
        DecimalFormat df = new DecimalFormat("#.00");
        String tempAmount = df.format(amount);
        amount = Double.parseDouble(tempAmount);
        int num = (int) amount;

        String paisa = " Only";
        if (amount - num != 0) {
            paisa = paisa( round((amount - num),2) ) + " Paisa Only";
        }

        String number = Integer.toString(num);
        String word = "";
        int length = number.length();
        if(length > 9){
        	String postFix = "00";
        	switch (length) {
			case 11:
				postFix += "0";
				break;
			case 12:
				postFix += "00";
				break;
			case 13:
				postFix += "000";
				break;
			}
        	if(length == 10){
        		word = numberToWordHelper(number.substring(0, length- 9)+postFix);
        	}else{
        		word = numberToWordHelper(number.substring(0, length- 9));
        	}
        	word += numberToWordHelper(number.substring(length- 9 , length));
        }else{
        	word = numberToWordHelper(number);
        }
        
        return (word + " Taka " + paisa);
    }
    
    public static double round(double number, int position) {
        double pow = 10;
        pow = Math.pow(pow, position);
        number = number * pow;
        number = Math.round(number);
        number = number / 100;
        return number;
    }
    
    public static String paisa(double paisa) {
        paisa *= 100;
        int num = (int) paisa;
        return "& " + ConvertOnesTwos(Integer.toString(num));
    }
    
    public static String ConvertOnesTwos(String t) {
        final String[] ones = {"", "One", "Two", "Three", "Four", "Five", "Six", "Seven", "Eight", "Nine", "Ten", "Eleven", "Twelve", "Thirteen", "Fourteen", "Fifteen", "Sixteen", "Seventeen", "Eighteen", "Nineteen"};
        final String[] tens = {"", "Ten", "Twenty", "Thirty", "Forty", "Fifty", "Sixty", "Seventy", "Eighty", "Ninety"};

        String word = "";
        int num = Integer.parseInt(t);
        if (num % 10 == 0) {
            word = tens[num / 10] + word;
        } else if (num < 20) {
            word = ones[num] + word;
        } else {
            word = tens[(num - (num % 10)) / 10] + word;
            word = word + ones[num % 10];
        }
        return word;
    }
    
    
    public static String numberToWordHelper(String number){
    	String twodigitword = "";
        String word = "";
        String[] HTLC = {"", "Hundred", "Thousand", "Lakh", "Crore"}; //H-hundread , T-Thousand, ..
        int split[] = {0, 2, 3, 5, 7, 9};
        String[] temp = new String[split.length];
        boolean addzero = true;
        int len1 = number.length();
        if (len1 > split[split.length - 1]) {
            System.out.println("Error. Maximum Allowed digits " + split[split.length - 1]);
        }
        for (int l = 1; l < split.length; l++) {
            if (number.length() == split[l]) {
                addzero = false;
            }
        }
        if (addzero == true) {
            number = "0" + number;
        }
        int len = number.length();
        int j = 0;
        //spliting & putting numbers in temp array.
        while (split[j] < len) {
            int beg = len - split[j + 1];
            int end = beg + split[j + 1] - split[j];
            temp[j] = number.substring(beg, end);
            j = j + 1;
        }

        for (int k = 0; k < j; k++) {
            twodigitword = ConvertOnesTwos(temp[k]);
            if (k >= 1) {
                if (twodigitword.trim().length() != 0) {
                    word = twodigitword + " " + HTLC[k] + " " + word;
                }
            } else {
                word = twodigitword;
            }
        }
        return word;
    }
    
}
